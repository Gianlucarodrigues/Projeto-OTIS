...
import "../../styles/kpi-elevadores.css";
    ...
    grupos: [
      {
        depto: "1) Vendas",
        kpis: [
          { label: "Faturamento do mês", unidade: "R$", meta: 10_000_000, atual: 8_300_000, dir: "up" },
      ...
      },
      {
        ...
        kpis: [
          ...
          { label: "Economia vs orçamento",     unidade: "%",    meta: 5.0, atual: 3.8, dir: "up" },
        ],
      },
      {
        depto: "4) Produção",
    ...
    grupos: [
      {
        depto: "1) Vendas",
        kpis: [
          { label: "Faturamento acumulado",     unidade: "R$", meta: 120_000_000, atual: 98_000_000, dir: "up" },
          { label: "Ticket médio por pedido",   unidade: "R$", meta: 220_000,     atual: 205_000,    dir: "up" },
        ],
      },
      {
        depto: "2) Engenharia",
        kpis: [
          ...
          { label: "Índice de retrabalho",      unidade: "%",  meta: 3.0, atual: 4.2, dir: "down" },
        ],
      },
      {
        depto: "3) Compras",
        kpis: [
          ...
          { label: "Savings acumulado",         unidade: "R$", meta: 3_000_000, atual: 2_200_000, dir: "up" },
        ],
      },
      {
        depto: "4) Produção",
        kpis: [
          ...
          { label: "Refugo/Sucata",             unidade: "%",  meta: 2.0, atual: 2.8, dir: "down" },
        ],
      },
      {
        depto: "5) Serviços de instalação",
...
/* ========= COMPONENTE ========= */
export default function DashboardElevadores({ titulo = "KPIs — Empresa de Elevadores" }) {
  ...
  return (
    <div className="page">
      <header className="top">
        <h1>{titulo}</h1>
        <div className="tabs">
          <button className={`tab ${aba === "mensal" ? "is-active" : ""}`} onClick={() => setAba("mensal")}>Mensal</button>
          <button className={`tab ${aba === "anual" ? "is-active" : ""}`} onClick={() => setAba("anual")}>Anual (YTD)</button>
        </div>
      </header>

      <h2 className="board-title">{board.titulo}</h2>

      {board.grupos.map((g) => (
        <section key={g.depto} className="sec">
          <h3 className="sec-title">{g.depto}</h3>
          <div className="grid">
            {g.kpis.map((k) => {
              const p = progresso(k);
              const st = statusClass(k); // ok | warn | danger
              return (
                <article key={k.label} className={`card ${st}`}>
                  <div className="head">
                    <span className="label">{k.label}</span>
                    <span className={`badge ${st}`}>{st === "ok" ? "No verde" : st === "warn" ? "Atenção" : "Crítico"}</span>
                  </div>

                  <div className="vals">
                    <div><strong className="val">{fmtValor(k.atual, k.unidade)}</strong><div className="sub">Atual</div></div>
                    <div><strong className="val">{fmtValor(k.meta, k.unidade)}</strong><div className="sub">Meta</div></div>
                  </div>

                  <div className="bar-outer">
                    <div className="bar-inner" style={{ width: `${Math.round(p * 100)}%` }} />
                  </div>

                  <div className="foot">
                    <span className="dir">Meta {k.dir === "down" ? "↓ menor" : "↑ maior"}</span>
